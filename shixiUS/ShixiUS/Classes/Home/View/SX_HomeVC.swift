...
class SX_HomeVC: SX_BaseControler {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        observation = self.homeTableView!.observe(\.contentOffset, options: [.new, .old]) {
            ...
            //根据偏移量修改导航栏透明度
            var delta = changed.newValue!.y / CGFloat(-kNavH) + 1
    ...
    override func viewDidLoad() {
        ...
        self.homeTableView = UITableView(frame: CGRect(x: 0, y: 0, width: SCREEN_WIDTH, height: SCREEN_HEIGHT-kNavH-kTabBarHeight), style: .plain)
